...
{
    std::string commitFile = ".minigit/commits/" + commitId + ".txt";
    std::ifstream in(commitFile);
    if (!in) {
        std::cerr << "Error: Commit " << commitId << " not found.\n";
        return;
    }
    ...
    while (std::getline(in, line)) {
        ...
        if (inFilesSection && line.rfind(" - ", 0) == 0) {
            std::string nameVer = line.substr(3); // remove " - "
            size_t parenPos = nameVer.find(" (v");
            if (parenPos != std::string::npos) {
                std::string name = nameVer.substr(0, parenPos);
                int version = std::stoi(nameVer.substr(parenPos + 3, nameVer.size() - parenPos - 4));
                files.push_back({name, version});
            }
        }
    }

    // Now restore each file
    for (const auto& [name, version] : files) {
        std::string src  = ".minigit/objects/" + name + "_" + std::to_string(version);
        std::string dest = name; // overwrite original
        if (copyFile(src, dest)) {
            std::cout << "Restored " << name << " (v" << version << ")\n";
        } else {
            std::cerr << "Failed to restore " << name << "\n";
        }
    }
...
void removeFile(const std::string& name) {
    auto it = std::remove_if(stagedFiles.begin(), stagedFiles.end(),
                             [&](const FileNode& f) { return f.fileName == name; });

    if (it != stagedFiles.end()) {
        stagedFiles.erase(it, stagedFiles.end());
        std::cout << "Unstaged: " << name << "\n";
    } else {
        std::cout << "File not staged: " << name << "\n";
    }
}
...
int main(int argc, char* argv[])
{
    ...
    if (cmd == "add") {
        ...
        addFile();                         // new function to add multiple files
    }
